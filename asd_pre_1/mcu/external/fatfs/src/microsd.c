...
void MICROSD_Init(void)
{
  ...
  CMU_ClockEnable(SD_USART_CLK, true);
  ...
  SD_USART->ROUTE = USART_ROUTE_TXPEN | USART_ROUTE_RXPEN |
                         USART_ROUTE_CLKPEN | SD_USART_LOC;

#if defined( USART_CTRL_SMSDELAY )
  /* This will allow us to use higher baudrate. */
  SD_USART->CTRL |= USART_CTRL_SMSDELAY;
#endif

  /* IO configuration */
  GPIO_PinModeSet(SD_TX_PORT, SD_TX_BIT, gpioModePushPull, 0);  /* MOSI */
  GPIO_PinModeSet(SD_RX_PORT, SD_RX_BIT, gpioModeInputPull, 1); /* MISO */
  GPIO_PinModeSet(SD_CS_PORT, SD_CS_BIT,   gpioModePushPull, 1);  /* CS */
  GPIO_PinModeSet(SD_CLK_PORT, SD_CLK_BIT,  gpioModePushPull, 0);  /* CLK */
}
...
{
  USART_Reset(SD_USART);

  /* IO configuration (USART 0, Location #0) */
  GPIO_PinModeSet(SD_TX_PORT, SD_TX_BIT, gpioModeDisabled, 0);  /* MOSI */
  GPIO_PinModeSet(SD_RX_PORT, SD_RX_BIT, gpioModeDisabled, 0);  /* MISO */
  GPIO_PinModeSet(SD_CS_PORT, SD_CS_BIT, gpioModeDisabled, 0);  /* CS */
  GPIO_PinModeSet(SD_CLK_PORT, SD_CLK_BIT, gpioModeDisabled, 0);  /* Clock */
}
...
{
  GPIO->P[ SD_CS_PORT ].DOUTSET = 1 << SD_CS_BIT; /* CS pin high. */
...
{
  GPIO->P[ SD_CS_PORT ].DOUTCLR = 1 << SD_CS_BIT; /* CS pin low. */
  if (WaitReady() != 0xFF)
  {
    MICROSD_Deselect();
    return 0;
...
void MICROSD_PowerOff(void)
{
  ...
  MICROSD_Deselect();
  /* Disable SPI clock */
  CMU_ClockEnable(SD_USART_CLK, false);
}
...
int MICROSD_BlockTx(const uint8_t *buff, uint8_t token)
{
  ...
  do
  {
    ...
    while (!(SD_USART->STATUS & USART_STATUS_TXBL));

    SD_USART->TXDOUBLE = val;
  } while (bc);
  ...
  while (!(SD_USART->STATUS & USART_STATUS_TXC));
...
{
  USART_BaudrateSyncSet(SD_USART, 0, MICROSD_LO_SPI_FREQ);
  xfersPrMsec = MICROSD_LO_SPI_FREQ / 8000;
}
...
{
  USART_BaudrateSyncSet(SD_USART, 0, MICROSD_HI_SPI_FREQ);
